...
const HomeTable = () => {
  ...
  return (
    <section className="sm:w-[440px] md:w-[440px] 2xl:w-full">
              ...
              {dropdownData.map((dropdown, index) => (
                <th key={index} className="w-full">
                  <DropDown
                    ...
                    selectStyles={`${dropdown.width} w-full border-white-1 text-[#D9D9D9] text-[14px] bg-white`}
                    insideStyles={`${dropdown.width} text-[#D9D9D9] bg-white`}
                    selectedItemRadius="rounded-none"
                    selectContainerStyles={`${dropdown.width}`}
                  />
                </th>
              ))}
            </tr>
            ...
            {data.map((row, rowIndex) => (
              <tr key={rowIndex} className="hover:bg-gray-100 ">
                {row.map((cell, cellIndex) => (
                  <td key={cellIndex} className="  py-2 text-center">
                    {cell}
                  </td>
                ))}
              </tr>
